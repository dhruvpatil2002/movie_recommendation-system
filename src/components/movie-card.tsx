import { ExternalLink, Film, Star, Users } from "lucide-react";
...
export function MovieCard({ movie, index }: MovieCardProps) {
  ...
  return (
    <Card className="flex h-full flex-col overflow-hidden border-white/80 bg-white/90 shadow-lg shadow-indigo-100/60 backdrop-blur-sm transition-all hover:-translate-y-1 hover:shadow-xl hover:shadow-violet-200/50">
      ...
      <CardContent className="flex flex-1 flex-col space-y-4">
        ...
        <div className="flex items-start gap-2 text-sm text-slate-600">
          ...
          <p className="line-clamp-2">{movie.cast.join(" · ")}</p>
        </div>
        ...
        <div className={`rounded-lg bg-gradient-to-br ${accent} p-px`}>
          <div className="rounded-[7px] bg-violet-50/80 px-3 py-2.5">
            ...
            <p className="mt-1 text-sm leading-relaxed text-slate-700 line-clamp-4">
              {movie.reason}
            </p>
          </div>
        </div>
