...
export function RecommendationApp() {
  const [userPrompt, setUserPrompt] = useState("Suggest movies for a rainy night");
    ...
    <div className="min-h-svh bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-violet-100 via-indigo-50 to-sky-50">
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <header className="mb-10 text-center">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/70 px-4 py-2 text-sm font-medium text-violet-700 shadow-sm ring-1 ring-violet-200/60 backdrop-blur">
          ...
          <h1 className="bg-gradient-to-r from-violet-700 via-indigo-600 to-sky-600 bg-clip-text text-4xl font-bold tracking-tight text-transparent sm:text-5xl">
            Movie Picks for Your Mood
          </h1>

          <p className="mx-auto mt-3 max-w-2xl text-sm text-slate-600 sm:text-base">
            Get tailored recommendations with posters, cast, reasons, and direct streaming links.
          </p>
        </header>
            ...
            {!loading && movies.length > 0 && (
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-slate-500">
                  {movies.length} movie{movies.length > 1 ? "s" : ""} matched your vibe. Streaming links open in a new tab.
                </p>
              </div>
            )}
            ...
            {!loading && !error && movies.length === 0 && !hasSearched && (
              <div className="flex min-h-[320px] flex-col items-center justify-center rounded-2xl border border-dashed border-violet-200/80 bg-white/50 px-6 text-center backdrop-blur-sm">
                ...
                <p className="mt-2 max-w-sm text-sm text-slate-500">
                  Set your genre, mood, and describe what you want — then hit Get recommendations.
                </p>
              </div>
            )}
